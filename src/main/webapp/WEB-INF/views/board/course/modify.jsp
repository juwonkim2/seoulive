...
<title>Course_Modify</title>

<link rel="stylesheet" href="https://kit.fontawesome.com/889f069cfd.css" crossorigin="anonymous">

<link rel="stylesheet" href="${pageContext.request.contextPath}/resources/css/course_write_copy.css">
...
<body>
	...
	<div id="header">
		<div class="wrap">
			<h1 class="logo">
				<a href="#"><img
					src="${pageContext.request.contextPath}/resources/img/img-logo.png"
	...
	<!-- ===================================================================================== -->
		...
		<div class="write">
			<div>
				...
				<div>작성자: ${__BOARD__.WRITER}</div>
				<div>
					제목: <input type="text" name="TITLE" placeholder="${__BOARD__.TITLE}" >
					<input type="hidden" name="ori_TITLE" value="${__BOARD__.TITLE}">
				</div>
				<div class="write_func">
					<label for="birthday">출발일자: <fmt:formatDate value="${__BOARD__.DURATION_START}" pattern="yyyy년 MM월 dd일" /></label>
					<input type="hidden" name="ori_DURATION_START" id="startDate" value="${__BOARD__.DURATION_START}">
				...
				<div>
					내용: <input type="text" name="REVIEW" placeholder="${__BOARD__.REVIEW}">
				</div>
			</div>
    ...
    <c:forEach begin="1" end="${extractedValue}" varStatus="loop">
				...
				<li>
					<div class="write_day" id="day${loop.count}">
						
						<div class="write_day_left">
							<span class="day_text">${loop.count}일차</span>
					<c:choose>
						...
						<div class="write_day_right">
							<ul class="dec_ul" id="list${loop.count}">
								<li>
									<div class="write_day_top">
										<span class="day_text2">Day${loop.count}. </span><span id="selectedDate${loop.count}"></span> 
									</div>
								</li>
						<c:forEach var="courseTravelVO" items = "${__COURSETRAVELBOARD__}" varStatus="loop2">
							<c:choose>
	    						<c:when test="${courseTravelVO.TRAVEL_ID.substring(0, 1) == loop.count}">
								<li>
								    <div class="write_day_dec_list" id="result${loop.count}_${loop2.count}">
								    	<div class="dec_img"><img src="../../../resources/img/seoul_forest.png" alt="" style="width: 80px; height: 50px;"></div>
										<div class="dec_title">여행지 : ${courseTravelVO.TITLE} / 테마 : ${courseTravelVO.CATEGORY}</div><br><br>
										<div class="dec_address">주소 : ${courseTravelVO.ADDRESS}</div>
										<input type="button" class="deleteButton" onclick="deleteElement(event)" value="삭제"><br>
										<input type="file" name="file" class="fileInput">
										<div class="planpage_course1review">${courseTravelVO.USER_REVIEW}</div>
							
								    </div>
								</li>
								<c:set var="resultNum" value="${courseTravelVO.TRAVEL_ID.substring(courseTravelVO.TRAVEL_ID.indexOf('_') + 1)+1}" />
								</c:when>
	             		   </c:choose>
	             		   
						</c:forEach>
								<li>
									<div class="write_day_dec_search" id="search${loop.count}_${resultNum}">
								        <input type="search" class="keyword" placeholder="검색어를 입력해주세요.">
								        <input type="button" value="검색" onclick="search(event)">
								        <input type="button" class="deleteButton" onclick="deleteElement(event)" value="삭제">
								    </div>
								    <div class="searchResult" id="result${loop.count}_${resultNum}">
								    </div>
								</li>
							</ul>
							<ul>
	...
	<script>
		...
		$(document).on('click', '.dec_plus', function() {
		    ...
		    var searchCount = $('#list' + dayCount + ' .write_day_dec_list').length + $('#list' + dayCount + ' .write_day_dec_search').length + 1; // 기존 write_day_dec_search 클래스 개수
		...
		$(document).on('click', '.day_plus', function() {
		    var dayCount = $('.write_day').length; // 기존 write_day 클래스 개수
		    var searchCount = $('#list' + dayCount + ' .write_day_dec_list').length + $('#list' + dayCount + ' .write_day_dec_search').length + 1; // 기존 write_day_dec_search 클래스 개수
		    var searchId = 'search' + dayCount + '_' + searchCount; // 고유한 id 값 생성
		    var resultId = 'result' + dayCount + '_' + searchCount;
		   ...
		   if(document.getElementById('startDateInput').value != null || document.getElementById('startDateInput').value != ''){
		...
		function handleSelectedData(seq, category, title, address, resultId) {
			...
			var img = $("<img>").attr("src", "../../../resources/img/seoul_forest.png")
			                     .attr("alt", "")
			                     .css("width", "80px")
			                     .css("height", "50px");
			imgDiv.append(img);
		...
		function deleteElement(event) {
		  ...
		  // 기존 여행지 창 삭제
		  $(".write_day_dec_list").each(function() {
			    var resultId = $(this).attr("id");
			    if (resultId === elementId) {
			     	$(this).css("display", "none");
			    }
		...
		function sendData() {
			// 전송할 데이터 추출
			var courseSeq = $('input[name=seq]').val();
			var writer = $('input[name="WRITER"]').val();
			
			if($('input[name="TITLE"]').val() != null || $('input[name="TITLE"]').val() != ""){
				var title = $('input[name="TITLE"]').val();
			} else {
				var title = $('input[name="ori_TITLE"]').val()
			}
			...
			alert("courseSeq: "+courseSeq+", writer: "+writer+", title: "+title+", durationStart: "+durationStart+", durationEnd: "+durationEnd+", review: "+review);
		  
			// AJAX 요청
			$.ajax({
				url: '/board/course/modify', 
				method: 'POST',
				data: {
					WRITER: writer,
				 TITLE: title,
	...
	<footer>
		<div class="footer_wrap">
			<div class="footer_box">
				<div class="logo_box">
					<a href="#a"><img
						src="/resources/static/img/img-footer-logo.png" /></a>
				</div>
				<div>
					<ul>
