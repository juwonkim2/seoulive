...
import org.zerock.seoulive.board.course.domain.CourseSearchTO;
...
public class CourseController {
	...
	void list(CoursePageTO page, Model model) throws ControllerException {
		...
		try {
			List<CourseVO> list = this.service.getList(page);
			...
			for (int i = 0; i<list.size(); i++) {
				...
				List<CourseTravelVO> list2 = this.service.getTravelList(list.get(i));
				model.addAttribute("__TRAVEL_LIST__", list2);
			}
	...
	// 2. 검색 후 게시판 목록 조회
		@GetMapping("/search")
		void search(CoursePageTO page, String searchType, String keyword, Model model) throws ControllerException {
			log.trace("list({}, {}) invoked.", page, model);
			
			try {
				List<CourseVO> list = this.service.searchCourse(page, searchType, keyword);
				...
				CoursePageDTO pageDTO = new CoursePageDTO(page, this.service.getTotalSearch(searchType, keyword));

				model.addAttribute("pageMaker", pageDTO);
				
				CourseSearchTO searchTO = new CourseSearchTO(searchType, keyword);
				
				model.addAttribute("search", searchTO);
